...
LIBC_INLINE float atanf16_poly_eval(float xsq) {
  float xsq2 = xsq * xsq;
  float xsq4 = xsq2 * xsq2;

  float p0 = fputil::multiply_add(xsq, -0x1.55519ep-2f,  0x1.fffffcp-1f);
  float p1 = fputil::multiply_add(xsq, -0x1.1f0a92p-3f,  0x1.98f6a8p-3f);
  float p2 = fputil::multiply_add(xsq, -0x1.e65492p-5f,  0x1.95b654p-4f);
  float p3 = fputil::multiply_add(xsq, -0x1.32316ep-8f,  0x1.8c0c36p-6f);

  float q0 = fputil::multiply_add(xsq2, p1, p0);
  float q1 = fputil::multiply_add(xsq2, p3, p2);

  return fputil::multiply_add(xsq4, q1, q0);
}
...
LIBC_INLINE constexpr float16 atanf16(float16 x) {
  ...
  if (x_abs <= 0x3c00) {
    ...
    // [0, 1]);
    float result = atanf16_poly_eval(xsq);
  ...
  // > P = fpminimax(atan(x)/x, [|0, 2, 4, 6, 8, 10, 12, 14|], [|SG...|],
  // [0, 1]);
  float interm = atanf16_poly_eval(x_inv_sq);
